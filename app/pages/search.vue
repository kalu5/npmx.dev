      ...
      <section v-if="query">
        ...
        <div v-else-if="visibleResults">
          ...
          >
            ...
            <button
              type="button"
              class="shrink-0 px-4 py-2 font-mono text-sm text-bg bg-fg rounded-md motion-safe:transition-colors motion-safe:duration-200 hover:bg-fg/90 outline-none focus-visible:(ring-2 ring-accent/90)"
          ...
          <div v-else-if="status === 'success' || status === 'error'" role="status" class="py-12">
            ...
            <div v-if="showClaimPrompt" class="max-w-md mx-auto text-center hidden sm:block">
              <div class="p-4 bg-bg-subtle border border-border rounded-lg">
                ...
                <button
                  type="button"
                  class="px-4 py-2 font-mono text-sm text-bg bg-fg rounded-md transition-colors duration-200 hover:bg-fg/90 outline-none focus-visible:(ring-2 ring-accent/90)"
                  @click="claimPackageModalRef?.open()"
                >
                  {{ $t('search.claim_button', { name: query }) }}
                </button>
