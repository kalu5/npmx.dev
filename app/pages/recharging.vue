    ...
    <article class="max-w-2xl mx-auto">
      <header class="mb-12">
        ...
        <div class="flex items-baseline justify-between gap-4 mb-4">
          ...
          <button
            type="button"
            class="cursor-pointer inline-flex items-center gap-2 font-mono text-sm text-fg-muted hover:text-fg transition-colors duration-200 rounded ring-focus-visible shrink-0"
            @click="router.back()"
            v-if="canGoBack"
          >
        ...
        >
          <div class="flex items-center gap-4 sm:gap-5 py-3 sm:py-4 w-max">
            <template v-for="n in 4" :key="`set-${n}`">
              ...
              <button
                type="button"
                class="relative shrink-0 cursor-pointer rounded transition-transform duration-200 hover:scale-110 ring-focus-visible w-5 h-5 sm:w-6 sm:h-6"
