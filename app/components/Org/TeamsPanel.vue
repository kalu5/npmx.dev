...
<template>
  <section v-if="isConnected" class="bg-bg-subtle border border-border rounded-lg overflow-hidden">
    ...
    <div class="flex items-center justify-start p-4 border-b border-border">
      ...
      <button
        type="button"
        class="p-1.5 text-fg-muted hover:text-fg transition-colors duration-200 rounded outline-none focus-visible:(ring-2 ring-accent/90)"
    ...
    <div class="flex items-center gap-2 p-3 border-b border-border bg-bg">
      ...
      >
        <button
          type="button"
          class="px-2 py-1 font-mono rounded transition-colors duration-200 outline-none focus-visible:(ring-2 ring-accent/90)"
    ...
    <div v-else-if="error" class="p-4 text-center" role="alert">
      ...
      <button
        type="button"
        class="mt-2 font-mono text-xs text-fg-muted hover:text-fg transition-colors duration-200 rounded outline-none focus-visible:(ring-2 ring-accent/90)"
        @click="loadTeams"
      >
        {{ $t('common.try_again') }}
      </button>
    ...
    <ul v-else class="divide-y divide-border" :aria-label="$t('org.teams.list_label')">
      <li v-for="teamName in filteredTeams" :key="teamName" class="bg-bg">
        ...
        >
          <button
            type="button"
            class="flex-1 flex items-center gap-2 text-start rounded outline-none focus-visible:(ring-2 ring-accent/90)"
          ...
          <button
            type="button"
            class="p-1 text-fg-subtle hover:text-red-400 transition-colors duration-200 rounded outline-none focus-visible:(ring-2 ring-accent/90)"
        ...
        >
          ...
          <div v-if="showAddUserFor === teamName" class="mt-2">
            <form class="flex items-center gap-2" @submit.prevent="handleAddUser(teamName)">
              ...
              <button
                ...
                class="px-2 py-1 font-mono text-xs text-bg bg-fg rounded transition-all duration-200 hover:bg-fg/90 disabled:opacity-50 disabled:cursor-not-allowed outline-none focus-visible:(ring-2 ring-accent/90)"
              >
                {{ isAddingUser ? '…' : $t('org.teams.add_button') }}
              </button>
              <button
                type="button"
                class="p-1 text-fg-subtle hover:text-fg transition-colors duration-200 rounded outline-none focus-visible:(ring-2 ring-accent/90)"
          ...
          <button
            v-else
            type="button"
            class="mt-2 px-2 py-1 font-mono text-xs text-fg-muted hover:text-fg transition-colors duration-200 rounded outline-none focus-visible:(ring-2 ring-accent/90)"
    ...
    <div class="p-3 border-t border-border">
      <div v-if="showCreateTeam">
        <form class="flex items-center gap-2" @submit.prevent="handleCreateTeam">
          ...
          <button
            ...
            class="px-3 py-2 font-mono text-xs text-bg bg-fg rounded transition-all duration-200 hover:bg-fg/90 disabled:opacity-50 disabled:cursor-not-allowed outline-none focus-visible:(ring-2 ring-accent/90)"
          >
            {{ isCreatingTeam ? '…' : $t('org.teams.create_button') }}
          </button>
          <button
            type="button"
            class="p-1.5 text-fg-subtle hover:text-fg transition-colors duration-200 rounded outline-none focus-visible:(ring-2 ring-accent/90)"
      ...
      <button
        v-else
        type="button"
        class="w-full px-3 py-2 font-mono text-sm text-fg-muted bg-bg border border-border rounded transition-colors duration-200 hover:text-fg hover:border-border-hover outline-none focus-visible:(ring-2 ring-accent/90)"
