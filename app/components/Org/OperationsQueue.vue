  ...
  <div v-if="isConnected" class="space-y-4">
    ...
    <div class="flex items-center justify-between">
      ...
      <div class="flex items-center gap-2">
        <button
          v-if="hasOperations"
          type="button"
          class="px-2 py-1 font-mono text-xs text-fg-muted hover:text-fg bg-bg-subtle border border-border rounded transition-colors duration-200 hover:border-border-hover outline-none focus-visible:(ring-2 ring-accent/90)"
        ...
        <button
          type="button"
          class="p-1 text-fg-muted hover:text-fg transition-colors duration-200 rounded outline-none focus-visible:(ring-2 ring-accent/90)"
      ...
      >
        ...
        <div class="flex-shrink-0 flex items-center gap-1">
          <button
            ...
            type="button"
            class="p-1 text-fg-muted hover:text-green-400 transition-colors duration-200 rounded outline-none focus-visible:(ring-2 ring-accent/90)"
          ...
          <button
            ...
            type="button"
            class="p-1 text-fg-muted hover:text-red-400 transition-colors duration-200 rounded outline-none focus-visible:(ring-2 ring-accent/90)"
    ...
    >
      ...
      <form class="flex flex-col gap-1" @submit.prevent="handleRetryWithOtp">
        <div class="flex items-center gap-2">
          ...
          <button
            ...
            class="px-3 py-2 font-mono text-xs text-bg bg-amber-500 rounded transition-all duration-200 hover:bg-amber-400 disabled:opacity-50 disabled:cursor-not-allowed outline-none focus-visible:(ring-2 ring-accent/90)"
          >
      ...
      <button
        type="button"
        :disabled="isExecuting"
        class="w-full px-3 py-2 font-mono text-xs text-fg bg-bg-subtle border border-border rounded transition-all duration-200 hover:text-fg hover:border-border-hover disabled:opacity-50 disabled:cursor-not-allowed outline-none focus-visible:(ring-2 ring-accent/90)"
    ...
    <div v-if="hasActiveOperations" class="flex items-center gap-2 pt-2">
      <button
        ...
        type="button"
        class="flex-1 px-4 py-2 font-mono text-sm text-fg bg-bg-subtle border border-border rounded-md transition-colors duration-200 hover:border-border-hover outline-none focus-visible:(ring-2 ring-accent/90)"
      ...
      <button
        ...
        type="button"
        :disabled="isExecuting"
        class="flex-1 px-4 py-2 font-mono text-sm text-bg bg-fg rounded-md transition-all duration-200 hover:bg-fg/90 disabled:opacity-50 disabled:cursor-not-allowed outline-none focus-visible:(ring-2 ring-accent/90)"
        @click="handleExecute()"
      >
        {{
          isExecuting
      ...
      <button
        v-if="authUrl"
        type="button"
        class="flex-1 px-4 py-2 font-mono text-sm text-accent bg-accent/10 border border-accent/30 rounded-md transition-colors duration-200 hover:bg-accent/20 outline-none focus-visible:(ring-2 ring-accent/90)"
    ...
    <details v-if="hasCompletedOperations" class="mt-4 border-t border-border pt-4">
      ...
      <ul class="mt-2 space-y-1" :aria-label="$t('operations.queue.log_label')">
        ...
        >
          ...
          <button
            type="button"
            class="p-0.5 text-fg-subtle hover:text-fg-muted transition-colors duration-200 rounded outline-none focus-visible:(ring-2 ring-accent/90)"
