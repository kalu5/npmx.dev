  ...
  <div v-if="chips.length > 0" class="flex flex-wrap items-center gap-2">
    <TransitionGroup name="chip">
      <TagStatic v-for="chip in chips" :key="chip.id" class="gap-2 pe-1">
        ...
        <button
          type="button"
          class="flex items-center p-1 -m-1 hover:text-fg rounded-full transition-colors duration-200 outline-none focus-visible:(ring-2 ring-accent/90)"
    ...
    <button
      v-if="chips.length > 1"
      type="button"
      class="text-sm p-0.5 text-fg-muted hover:text-fg underline transition-colors duration-200 outline-none focus-visible:(ring-2 ring-accent/90)"
      @click="emit('clearAll')"
    >
      {{ $t('filters.clear_all') }}
    </button>
