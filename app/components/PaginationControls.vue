  ...
  >
    ...
    <div class="flex items-center gap-4">
      ...
      >
        <button
          type="button"
          class="px-2.5 py-1 text-xs font-mono rounded-sm transition-colors duration-200 ring-focus-visible"
    ...
    <div v-if="effectiveMode === 'paginated'" class="flex items-center gap-4">
      ...
      >
        ...
        <button
          type="button"
          class="p-1.5 rounded hover:bg-bg-muted text-fg-muted hover:text-fg disabled:opacity-40 disabled:cursor-not-allowed transition-colors duration-200 ring-focus-visible"
          :disabled="!canGoPrev"
        ...
        <template v-for="(page, idx) in visiblePages" :key="idx">
          ...
          <button
            v-else
            type="button"
            class="min-w-[32px] h-8 px-2 font-mono text-sm rounded transition-colors duration-200 ring-focus-visible"
            :class="
              page === currentPage
                ? 'bg-fg text-bg focus-visible:(bg-accent/50 text-accent)'
